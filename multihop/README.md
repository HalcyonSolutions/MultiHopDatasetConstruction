# Halcyon notes (How-to)
Notes: Everything thats under `FbWiki` is ours

- `pip install -r requirements.txt`
- install torch

### To run simulations on our dataset
Files that are related to our dataset:
`
fbwiki-complex.sh
fbwiki-distmult.sh           
fbwiki_filtered-conve.sh     
fbwiki_filtered-rs.sh        
fbwiki-rs.sh
fbwiki-conve.sh              
fbwiki_filtered-complex.sh   
fbwiki_filtered-distmult.sh  
fbwiki_filtered.sh           
fbwiki.sh
`

- Run the following command to preprocess the dataset: `./experiment.sh configs/fbwiki.sh --process_data 0`. Note that you must have these files in order to pre-process the data: `raw.pgrk`, `train.triples`, `test.triples`, `dev.triples`, `raw.kb`. I have some codes that I wrote in the FbWiki directory to create those, but they are not of a good quality because I was writing them super fast just to make things work.
- Train embedding-based models: `./experiment-emb.sh configs/fbwiki-dismult.sh --train 0`
- Train RL models (policy gradient) (I didn't train those): `./experiment.sh configs/fbwiki.sh --train 0`
- Train RL models (policy gradient + reward shaping): `./experiment-rs.sh configs/fbwiki-rs.sh --train 0`

### To run inference
Just replace `--train` to `--inference`
