...
def print_paths_qid(paths) -> None:
    for path in paths:
        nodes = path[0]
        rels = path[1]
...
if __name__ == '__main__':
    ...
    nodesD, relsD = g.match_related_nodes(node, direction='<-', qid_only=True)   # extracting nodes to which point to this one
    ...
    paths = g.find_path(start_node, end_node, min_hops=min_hops, max_hops=max_hops, limit=limit, relationship_types=rels, qid_only=True)
    print_paths_qid(paths)
    ...
    max_degree = 1
    limit = 0
    neighborhood = g.find_neighborhood(qid_list=nodes, max_degree=max_degree, limit=limit, qid_only=True)
    
    #--------------------------------------------------------------------------
    'Relationships between Nodes'
    
    start_node = 'Q76'                       # Barack Obama
    end_node   = 'Q22686'                    # Donald Trump
